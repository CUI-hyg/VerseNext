...
### VMPC V1.5 压缩技术（Part5K1 新增）

Part5K1 正式命名模型参数压缩技术为 **VMPC（Versenext Model Parameters Compression）**，升级到 V1.5。`verse_torch.vmpc` 作为压缩 / 量化 / 蒸馏 / 剪枝的统一门面，re-export `compress.py` 核心对象（保证 `verse_torch.vmpc.compress_pipeline is verse_torch.compress.compress_pipeline`）。详见 [ADR-013](docs/architecture/adr-013-vmpc-naming-v15.md)。

- **V1.5 在 V1.3 基础上新增三项算法升级**：
  - `contrastive_distill`（对比蒸馏）：margin ranking loss 保证 student / teacher top-k 排序一致，避免"分布近似但 top-1 漂移"。
  - `logit_calibration`（推理 logits 校准）：temperature-aware sharpening，弥补量化后 logits 尺度漂移，校准因子存到模型 `_vmpc_logit_calib_factor` 元数据。
  - `outlier-aware` 反量化：outlier channel 保留 fp16，其余 int4，反量化时对 outlier 通道叠加 fp32 residual。
- **VMPCRegularizer**：防过拟合 + 压缩感知稀疏收紧。`step(val_loss)` 在 val_loss 平台期自动收紧 `target_sparsity *= sparsity_decay`，降到 `SPARSITY_FLOOR`（0.05）触发早停。
- **`vmpc_compress(model, profile="small"|"mate")`**：一键压缩预设，small = ternary + 高稀疏 0.5，mate = int4 + 中稀疏 0.3 + LoRA + 蒸馏。
- **版本分派**：`compress_pipeline(version="1.5")` 走 V1.5 路径，`version="1.3"` 仍可访问 V1.3（向后兼容）。V1.5 stats 与 V1.3 同构 + 追加 `vmpc_version` / `logit_calib_factor` / `contrastive_distill` 字段。

```python
from verse_torch.vmpc import vmpc_compress, VMPCRegularizer, compress_pipeline

# 一键预设
compressed = vmpc_compress(model, profile="mate",
                           teacher_model=teacher, train_loader=loader)

# 显式 V1.5 路径
compressed, stats = compress_pipeline(model, config, version="1.5", return_stats=True)

# VMPCRegularizer 接入训练循环
...
### 双模型并行 small / mate（Part5K1 新增）

Part5K1 正式开始双模型并行，`spark/small/` 与 `spark/mate/` 分别承载两个定位差异明显的模型。详见 [ADR-014](docs/architecture/adr-014-dual-model-small-mate.md)。

- **spark/small/**：0.06zB 小模型（1zB ≈ 1000B 等效能力），面向端侧 / 嵌入式 / 树莓派。VMPC-small 预设：ternary 量化 + 高稀疏 0.5。
- **spark/mate/**：0.2zB 旗舰模型，面向消费级 CPU / 单卡 GPU。VMPC-mate 预设：int4 量化 + 中稀疏 0.3 + LoRA + 蒸馏。
- **统一基座**：双模型均基于 `verse_nex.CometSparkNexLM`，VMPC 适配通过 config 传入。
- **checkpoint 目录**：`mf_small/` / `mf_mate/`（替代旧 `checkpoints_XXX/`），`mf_` 前缀统一。
- **模型文件默认 `.vn`**：性能优化格式（safetensors mmap 零拷贝 + pickle-free 安全）。
- **`spark/run.py` 支持 `--model small|mate`**：`python spark/run.py train --model small`，checkpoint 写 `mf_small/`。

```bash
# 训练 small 模型（0.06zB，端侧部署）
python spark/run.py train --model small

# 训练 mate 模型（0.2zB，旗舰能力）
python spark/run.py train --model mate

# spark/run.py 子命令均支持 --model（finetune/posttrain/continue）
python spark/run.py finetune --model mate --method lora
...
| 大→小模型能力转移 | **VMPC V1.5**（prune → quantize → distill → lora + contrastive_distill + logit_calibration + outlier-aware） | 三重损失 + 对比蒸馏 + 推理校准 + outlier 反量化；详见 [ADR-012](docs/architecture/adr-012-compression-v13.md) / [ADR-013](docs/architecture/adr-013-vmpc-naming-v15.md) |
...
| 包 | 文档 | 定位 |
|---|---|---|
| VerseTorch | [README](packages/verse_torch/README.md) | 张量 / autograd / vnn / optim / losses / training / quantize / parallel / compress / DeviceBackend（GPU/NPU）/ **.vn 格式（Part4K2）** / **LayerWiseTrainer（Part4K2）** / **compress V1.3（Part4K2）** / **vnn 重命名 + VMPC V1.5 + VMTTrainer + jsonl_repair（Part5K1）** |
| VerseNex | [README](packages/verse_nex/README.md) | Mamba-2 / RWKV-7 / RetNet / Sparse Attention / Hybrid / TriSparseAttention / MoDLayer / VerseNexLM / NexRL / **compress_v13 / distill_from（Part4K2）** / **VMPC V1.5 适配（Part5K1）** |
...
| CometSpark | [README](spark/README.md) | CometSpark V0.5-1B 模型说明 + 配置 + 训练 CLI（Part4K1）/ **持续训练 verse-continue（Part4K2）** / **双模型并行 small/mate + --model 参数（Part5K1）** |
...
│   │       ├── compress.py         # VMPC 压缩管线 V1.5（prune→quantize→distill→lora，Part4K2/Part5K1）
│   │       ├── vmpc.py            # VMPC 门面 + VMPCRegularizer + vmpc_compress（Part5K1）
...
├── spark/                  # CometSpark 端到端 LM 训练仓库（Part4K1，Part5K1 双模型并行）
│   ├── _bootstrap.py       # 统一路径引导模块（Part4K2.5）
│   ├── run.py              # CLI 快捷入口（train/eval/generate/chat/compress/convert/download + finetune/posttrain/continue，--model small|mate）
│   ├── small/              # 0.06zB 小模型（VMPC-small 预设，Part5K1）
│   ├── mate/               # 0.2zB 旗舰模型（VMPC-mate 预设，Part5K1）
