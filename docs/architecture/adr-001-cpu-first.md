...
本 ADR 确立的 **CPU 优先原则进一步深化**，Part4K2 围绕"让大模型在 CPU 上可训练 / 可部署"推出三项关键能力：

- **.vn 文件格式**（[ADR-009](adr-009-vn-format.md)）：基于 safetensors 的 ZIP 容器，mmap 零拷贝读取 + pickle-free 安全性，safetensors 不可用时降级 npz（纯标准库 + numpy），符合"零重型依赖"原则。
- **智能分区训练**（[ADR-011](adr-011-layerwise-training.md)）：`LayerWiseTrainer` 按 layer 分组训练 + `.vn` 分片卸载，让 1B 参数模型在 8GB 内存 CPU 上可训练（全量训练需 12GB+），无需分布式 / 通信库依赖。
- **压缩技术 V1.3**（[ADR-012](adr-012-compression-v13.md)）：`prune → quantize → distill → lora` 流水线 + 三重损失知识蒸馏，把大模型能力转移到 INT4 小模型，端侧 CPU 推理吞吐率提升约 4×。

此外，`verse_torch.device` 完善了 `get_memory_info` / `set_num_threads` / `auto_tune_threads` / `empty_cache` 等 CPU 资源管理 API，`VerseNexBlock` 新增 `use_checkpoint` 激活检查点开关（CPU 自动降级为直接前向）。
