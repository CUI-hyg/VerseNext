...
# CLI 启用
verse-train --config spark/mate/config/cometspark_mate.yml --device cuda    # GPU
verse-train --config spark/mate/config/cometspark_mate.yml --device npu     # NPU
```
...
## 12. VMPC V2.0 压缩技术调优（Part5K1.1 大升级）

Part5K1.1 将 **VMPC（VerseNext Model Parameters Compression）** 全面升级到 V2.0，**全面抛弃 V1.5 技术栈**。VMPC V2.0 明确 **VMPC ≠ 单纯的量化 / 剪枝**，而是基于 **VN 格式文件 + 传统技术 + VSC（VerseNext Space Compression）** 三大支柱。`verse_torch.vmpc` 提供 V2.0 压缩 API，`verse_torch.vsc` 提供 VSC 三维空间压缩引擎。详见 [ADR-013](architecture/adr-013-vmpc-naming-v15.md)。

> **等效能力**：在 VMPC V2.0 支持下，**1zB ≈ 1010B 模型能力**（等效处理）。物理压缩占 40%，算法优化 + 训练补偿占 60%。

### 12.1 何时使用 VMPC

- 大模型已训练完成，希望通过 VSC 三维空间压缩部署到端侧
- 需要在存储 / 算力 / 时间三维同时优化
- 希望压缩后通过训练补偿恢复能力
- `use_vmpc=True`（默认）时强制 `*.vn` 格式，获得多空间缓存加速

### 12.2 VMPC V2.0 一键压缩（Part5K1.1）

```python
from verse_torch.vmpc import vmpc_compress

# VMPC V2.0 路径（默认）：VSC 引擎 + 强制 .vn + 训练补偿
...
VSC 从存储 / 算力 / 时间三维权重优化压缩策略：

```python
from verse_torch.vsc import VSCConfig
...
| 维度 | 建议 |
|---|---|
| `target_ratio` | small 用 0.06，mate 用 0.2；过低会破坏模型结构 |
| `quantize_bits` | 端侧部署用 2（ternary），平衡用 4（INT4） |
| `storage_weight` | 小模型存储优先，建议 0.5；大模型可降低 |
| `compute_weight` | 算力受限场景提高（0.4 ~ 0.5） |
| `time_weight` | 实时推理场景提高（0.3 ~ 0.4） |
| `enable_compensation` | 默认 True；训练补偿恢复压缩损失的能力（算法优化 60%） |

### 12.5 吞吐率优化

VSC 量化后 INT4 / ternary 权重走 fused matmul 路径，访存优势转化为实际吞吐率提升：
...
> **提示**：VMPC V2.0 的 VN 多空间缓存（内存 LRU + 硬盘 mmap）进一步提升大模型加载吞吐率。

### 12.6 训练补偿（Part5K1.1 新增）

`VMPCV2.compensate()` 通过 `train_fn` 恢复压缩损失的能力：
...
print(f"recovered={result['recovered']:.2%}")
```
...
训练完成后用 VMPC V2.0 压缩管线部署到端侧（Part5K1.1 起统一品牌为 VMPC V2.0，默认走 VSC 引擎 + 强制 `.vn` 格式）：

```python
from verse_torch.vmpc import vmpc_compress

# VMPC V2.0 路径（默认）：VSC 三维空间压缩 + 强制 .vn + 训练补偿
small_model, stats = vmpc_compress(
    large_model, profile="small", use_vmpc=True,
    compensate_fn=train_fn,  # 可选：训练补偿恢复能力
)
# 压缩后约 280M 参数（INT4），峰值内存 ~1.5 GB，可在消费级 CPU 上推理
# use_vmpc=True 时自动强制 .vn 格式输出
```

Legacy 路径（`use_vmpc=False`）走 :func:`verse_torch.compress.compress_pipeline` 传统技术直通（prune / quantize / lora / ternary / distill），允许 `.pt` 格式。
...
- [ ] 部署：`vmpc_compress(use_vmpc=True)` VMPC V2.0 压缩 + 强制 `.vn` 格式
