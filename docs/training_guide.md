...
20. [VMPC V1.5 压缩指南（Part5K1 新增）](#20-vmpc-v15-压缩指南part5k1-新增)
21. [双模型训练指南（Part5K1 新增）](#21-双模型训练指南part5k1-新增)
...
## 20. VMPC V1.5 压缩指南（Part5K1 新增）

Part5K1 正式命名模型参数压缩技术为 **VMPC（Versenext Model Parameters Compression）**，升级到 V1.5。`verse_torch.vmpc` 作为压缩 / 量化 / 蒸馏 / 剪枝的统一门面。详见 [ADR-013](architecture/adr-013-vmpc-naming-v15.md)。

### 20.1 VMPC 门面与 re-export

`verse_torch.vmpc` re-export `compress.py` 的核心对象，保证调用者通过 `vmpc` 与通过 `compress` 拿到的是同一对象：

```python
from verse_torch.vmpc import (
    compress_pipeline, compression_report,
    OutlierSafePruner, LoRALinear, KnowledgeDistiller, QLinear,
    VMPCRegularizer, vmpc_compress,
)

# 等价性：vmpc.compress_pipeline is compress.compress_pipeline
assert __import__("verse_torch.vmpc", fromlist=["compress_pipeline"]).compress_pipeline \
    is __import__("verse_torch.compress", fromlist=["compress_pipeline"]).compress_pipeline
```
...
- **contrastive_distill（对比蒸馏）**：margin ranking loss 保证 student / teacher top-k 排序一致。
- **logit_calibration（推理 logits 校准）**：temperature-aware sharpening，校准因子存到模型 `_vmpc_logit_calib_factor` 元数据。
- **outlier-aware 反量化**：outlier channel 保留 fp16，其余 int4。

```python
from verse_torch.vmpc import compress_pipeline

config = {
    "prune":    {"sparsity": 0.3},
    "quantize": {"bits": 4, "outlier_aware": True, "outlier_ratio": 0.01},
    "distill":  {
        "teacher": teacher_model, "train_loader": train_loader,
        "epochs": 3, "lr": 1e-3, "temperature": 4.0,
        "distill_contrastive": True,           # V1.5 对比蒸馏
        "contrastive_loss_weight": 0.5,
        "contrastive_margin": 0.5,
        "contrastive_top_k": 10,
    },
    "lora":     {"rank": 8, "alpha": 16},
    "logit_calibration": True,                  # V1.5 推理 logits 校准
}
compressed, stats = compress_pipeline(model, config=config, version="1.5", return_stats=True)
print(stats["vmpc_version"])   # "1.5"
print(stats["logit_calib_factor"])
print(stats["contrastive_distill"])
```

### 20.3 VMPCRegularizer 接入训练循环

`VMPCRegularizer` 提供防过拟合 + 压缩感知稀疏收紧，val_loss 平台期自动收紧 sparsity：
...
reg = VMPCRegularizer(
    l2_weight=1e-5,
    dropout_rate=0.0,           # 压缩感知 dropout（0=不启用）
    target_sparsity=0.3,        # 初始目标稀疏度
    patience=5,                 # 连续 5 步 val_loss 不降则收紧
    sparsity_decay=0.9,         # 每次收紧到原值 90%
)
reg.attach(trainer)             # monkey-patch _compute_loss 加入正则项
...
for step in range(max_steps):
    ...
    if step % eval_interval == 0:
        ...
        print(f"step={step} val_loss={val_loss:.4f} sparsity={new_sparsity:.3f}")
        if should_stop:
            print(f"sparsity 触底（{reg.SPARSITY_FLOOR}），早停")
            break
```

### 20.4 vmpc_compress 一键预设

`vmpc_compress(model, profile="small"|"mate")` 提供双模型场景的一键压缩预设：

```python
from verse_torch.vmpc import vmpc_compress

# small 预设：ternary + 高稀疏 0.5（适配 0.06zB 小模型）
small_compressed = vmpc_compress(model, profile="small")

# mate 预设：int4 + 中稀疏 0.3 + LoRA（适配 0.2zB 旗舰模型）
# 蒸馏需用户显式传 teacher_model
mate_compressed = vmpc_compress(
    model, profile="mate",
...
V1.3 仍可通过 `version="1.3"` 访问（向后兼容）：

```python
# V1.3 路径（向后兼容）
compressed, stats = compress_pipeline(model, config=config, version="1.3")
print(stats["vmpc_version"])   # "1.3"
```

---

## 21. 双模型训练指南（Part5K1 新增）

Part5K1 正式开始双模型并行，`spark/small/` 与 `spark/mate/` 分别承载 0.06zB 小模型与 0.2zB 旗舰模型。详见 [ADR-014](architecture/adr-014-dual-model-small-mate.md)。

### 21.1 模型定位

| 模型 | 目录 | 能力档位 | VMPC 预设 | 定位 |
|---|---|---|---|---|
| small | `spark/small/` | 0.06zB（≈60B 等效能力） | ternary + 高稀疏 0.5 | 端侧 / 嵌入式 / 树莓派 |
| mate | `spark/mate/` | 0.2zB（≈200B 等效能力） | int4 + 中稀疏 0.3 + LoRA + 蒸馏 | 消费级 CPU / 单卡 GPU |

> 1zB ≈ 1000B 等效能力，用于横向对比不同压缩比下的等效能力。

### 21.2 CLI 用法

```bash
# 训练 small 模型（checkpoint 写 mf_small/）
python spark/run.py train --model small

# 训练 mate 模型（checkpoint 写 mf_mate/）
python spark/run.py train --model mate

# 不指定 --model 时保留默认行为（向后兼容）
python spark/run.py train
```
...
checkpoint 默认输出 `.vn` 格式（safetensors mmap 零拷贝 + pickle-free 安全），`.pt` 格式可通过 `verse-convert` 互转。
