...
class TestYAMLConfigFiles:
    """spark/{small,mate}/config/ 下的 YAML 配置文件正确加载。

    Part5K1.1：旧 ``spark/config/`` 已删除，配置迁移到
    ``spark/small/config/cometspark_small.yml`` 与 ``spark/mate/config/cometspark_mate.yml``。
    """

    def test_load_v05_yml(self):
        """cometspark_mate.yml 正确加载为 CometSparkMateConfig（旗舰规模，与旧 V05 1B 一致）。"""
        from spark.mate.model.config import CometSparkMateConfig

        yml_path = _REPO_ROOT / "spark" / "mate" / "config" / "cometspark_mate.yml"
        cfg = CometSparkMateConfig.from_yaml(str(yml_path))
        ...
        assert cfg.vmpc_target_ratio == 0.20

    def test_load_v05_small_yml(self):
        """cometspark_small.yml 正确加载。"""
        from spark.small.model.config import CometSparkSmallConfig

        yml_path = _REPO_ROOT / "spark" / "small" / "config" / "cometspark_small.yml"
        cfg = CometSparkSmallConfig.from_yaml(str(yml_path))
    ...
    def test_v05_yml_model_constructs(self):
        """从 cometspark_small.yml 构造模型（小 vocab 避免 OOM）。"""
        from spark.small.model.config import CometSparkSmallConfig
        from spark.small.model.model import CometSparkSmallLM
