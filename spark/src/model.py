...
class CometSparkV05LM:
    ...
    Class attributes:
        _config_class: 该 LM 对应的 Config 类，子类（Small/Mate）覆盖以保留
            VMPC 字段。供 ``from_pretrained`` / ``load_vn`` 健壮加载时使用。
    """

    # 子类覆盖为各自的 Config 类（保留 VMPC 字段）
    _config_class = CometSparkV05Config
    ...
    def __init__(self, config: CometSparkV05Config):
        ...
        self.net = CometSparkNexLM(
            ...
            mod_version=getattr(config, "mod_version", "1.2"),
            mod_entropy_weight=getattr(config, "mod_entropy_weight", 1e-3),
    ...
    def save(self, path: str, **kwargs) -> None:
        ...
        Part5K1.1：VMPC 启用时（``config.enforce_vn_format()=True``）强制走
        ``.vn`` 格式（VSC 多空间缓存 + 压缩感知布局），传入 ``.pt`` 路径会被
        自动改写为同名 ``.vn``；legacy 模式下保留原 ``.pt`` pickle 行为。

        Args:
            path: 输出文件路径（``.pt`` 或 ``.vn``）。
            **kwargs: ``.vn`` 模式下透传给 :meth:`save_vn`
                （如 ``chat_template`` / ``tokenizer``）。
        """
        # Part5K1.1：VMPC 启用 → 强制 .vn（避免破坏 V2 多空间缓存 + 压缩布局）
        if self.config.enforce_vn_format():
            if not path.lower().endswith(".vn"):
                # 自动改写后缀：xxx.pt / xxx → xxx.vn
                base = path[:-3] if path.lower().endswith(".pt") else path
                path = base + ".vn"
            self.save_vn(path, **kwargs)
            return

        # legacy 模式：.pt pickle（向后兼容）
    ...
    def load(self, path: str) -> "CometSparkV05LM":
        """从 ``.pt`` / ``.vn`` 文件加载 state_dict 到当前模型（config 不变）。

        Part5K1.1：自动识别文件后缀，``.vn`` 走 ``VNFileReader`` 零拷贝读取；
        ``.pt`` 走 pickle 路径（legacy）。
        """
    ...
    @classmethod
    def from_pretrained(cls, path: str, config_path: Optional[str] = None) -> "CometSparkV05LM":
        """从目录或单文件加载完整模型（Part5K1.1：健壮加载）。

        解决旧版 ``KeyError: 'config'``：训练保存的 ``best.pt`` / ``resume.pt``
        是纯 state_dict（无 config），旧实现直接 ``payload["config"]`` 崩溃。
        本版按优先级探测多种格式，config 缺失时回落到 ``config_path`` → 同目录
        ``config.yml`` → 类默认配置（``strict=False`` 加载）。

        加载优先级：
        - **目录**：``model.vn`` > ``model.pt``（含 config） > ``best.pt``
              （纯 state_dict，需 ``config.yml`` 重建）
        - **单文件**：
            - ``*.vn`` → ``load_vn``（含 config，无损）
            - ``*.pt`` → payload 含 ``config`` 走完整加载；纯 state_dict 走
              ``config_path`` / 同目录 ``config.yml`` / 默认 config 重建

        Args:
            path: 目录或文件路径（``.vn`` / ``.pt``）。
            config_path: 配置文件路径或目录（可选）。加载纯 state_dict 时用于
                重建模型；None 则尝试 ``path`` 同目录的 ``config.yml``，再回落
                到 ``cls._config_class()`` 默认配置。
        ...
        # ------------------------------------------------------------------
        # 目录模式：model.vn > model.pt > best.pt（+ config.yml 重建）
        # ------------------------------------------------------------------
        if os.path.isdir(path):
            # config 优先从目录自身 config.yml 读取，否则 config_path，否则默认
            config = cls._resolve_config(path, config_path, cfg_cls)
            model = cls(config)

            model_vn = os.path.join(path, "model.vn")
            model_pt = os.path.join(path, "model.pt")
            best_pt = os.path.join(path, "best.pt")

            if os.path.exists(model_vn):
                # .vn 含完整 config + 权重，直接走 load_vn（更准）
            ...
            if os.path.exists(best_pt):
                ...
                model.load_state_dict(sd, strict=False)
                return model
            raise FileNotFoundError(f"目录中未找到 model.vn / model.pt / best.pt：{path}")
        ...
        # .pt 文件：可能含 config，也可能是纯 state_dict（best.pt / resume.pt）
        with open(path, "rb") as f:
            payload = pickle.load(f)

        if isinstance(payload, dict) and "config" in payload:
            # 完整 payload：{"arch":..., "config":..., "state_dict":...}
            config = cfg_cls.from_dict(payload["config"])
            model = cls(config)
            sd = payload.get("state_dict", payload)
            model.load_state_dict(sd, strict=False)
            return model

        # 纯 state_dict（best.pt / resume.pt / save_pretrained 的 model.pt）
        # 旧版在此处 payload["config"] 崩溃 → 现在回落到 config_path / 同目录 config.yml / 默认
        sd = payload.get("model_state_dict", payload) if isinstance(payload, dict) else payload
        if isinstance(sd, dict) and "state_dict" in sd:
            sd = sd["state_dict"]
        config = cls._resolve_config(os.path.dirname(path) or ".", config_path, cfg_cls)
        model = cls(config)
        model.load_state_dict(sd, strict=False)
        return model

    @classmethod
    def _resolve_config(cls, dir_path: str, config_path: Optional[str], cfg_cls) -> "CometSparkV05Config":
        """解析配置：config_path > dir_path/config.yml > 类默认配置。

        config_path 支持三种形式：
        - 目录（含 ``config.yml``）→ ``from_pretrained(dir)``
        - ``.yml`` / ``.yaml`` 文件 → ``from_yaml(file)``
        - 其他（忽略，回落）
        """
        # 1. 显式 config_path
        if config_path:
            if os.path.isdir(config_path):
                try:
                    return cfg_cls.from_pretrained(config_path)
                except Exception:
                    pass
            elif os.path.isfile(config_path) and config_path.lower().endswith((".yml", ".yaml")):
                try:
                    return cfg_cls.from_yaml(config_path)
                except Exception:
                    pass
        # 2. 同目录 config.yml
        sibling_cfg = os.path.join(dir_path, "config.yml")
        if os.path.exists(sibling_cfg):
            try:
                return cfg_cls.from_pretrained(dir_path)
            except Exception:
                pass
        # 3. 类默认配置
        return cfg_cls()

    def save_pretrained(self, dir_path: str, **kwargs) -> None:
        """保存到目录（HuggingFace 风格）。

        Part5K1.1：VMPC 启用时生成 ``model.vn``（VSC + .vn 多空间缓存），
        legacy 模式生成 ``model.pt``（pickle，向后兼容）。

        生成：
            dir_path/
              config.yml      ← CometSparkV05Config（model 段）
              model.vn | model.pt   ← 权重（按 VMPC 模式自动选择）
        """
        os.makedirs(dir_path, exist_ok=True)
        # 1. config.yml
        self.config.save_pretrained(dir_path)
        # 2. 权重（VMPC 启用 → .vn，否则 → .pt）
        if self.config.enforce_vn_format():
