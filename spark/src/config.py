...
class CometSparkV05Config:
    ...
    Args / 字段：
        ...
        mod_version: MoD 路由版本（Part5K1.1 默认 ``"1.2"``，启用输入 LayerNorm +
            熵正则 + 稳定初始化）。仅在 ``layer_pattern`` 含 ``"mod"`` 时生效。
        mod_entropy_weight: MoD V1.2 路由熵正则权重（默认 ``1e-3``）。
        vmpc_enabled: VMPC 总开关（Part5K1.1 默认 ``True``）。``True`` 启用
            VMPC V2（VSC + .vn 容器 + 传统压缩原子），强制 ``.vn`` 持久化；
            ``False`` 走 legacy ``.pt`` 路径（向后兼容旧 checkpoint）。
        vmpc_version: VMPC 压缩管线版本（默认 ``"2.0"``）。legacy 模式下可降到
            ``"1.5"`` / ``"1.3"`` / ``"1.0"``；VMPC 启用时强制 ``"2.0"``。
    """
    ...
    # Part5K1.1：MoD V1.2 路由升级（路由输入 LayerNorm + 熵正则 + 稳定初始化）
    mod_version: str = "1.2"
    mod_entropy_weight: float = 1e-3

    # Part5K1.1：VMPC 总开关 + 模式分组
    # - vmpc_enabled=True（默认）：启用 VMPC V2（VSC + .vn 容器 + 传统压缩原子）；
    #   所有模型权重必须以 ``*.vn`` 格式持久化（save/load 自动走 .vn 路径），
    #   不允许降级为 .pt（避免破坏 VMPC V2 的多空间缓存与压缩感知布局）。
    # - vmpc_enabled=False（legacy）：禁用 VMPC，回退到 legacy .pt + 旧压缩原子
    #   （prune/quantize/lora/ternary/distill 仍可用，但不与 VSC/.vn 联动）。
    vmpc_enabled: bool = True
    # VMPC V2 压缩管线版本：``"2.0"`` 走 VSC + VN 联动新管线；
    # ``"1.5"`` / ``"1.3"`` / ``"1.0"`` 仅在 legacy 模式下生效。
    vmpc_version: str = "2.0"
    ...
    def is_vmpc_enabled(self) -> bool:
        """返回是否启用 VMPC（True=V2 新管线，False=legacy 旧路径）。"""
        return bool(self.vmpc_enabled)

    def enforce_vn_format(self) -> bool:
        """VMPC 启用时是否强制 ``.vn`` 格式持久化。

        - VMPC 启用 → 必须 ``.vn``（保证 VSC 多空间缓存与压缩感知布局生效）
        - legacy 模式 → 允许 ``.pt``（向后兼容旧 checkpoint）
        """
        return self.is_vmpc_enabled()

    def get_vmpc_version(self) -> str:
        """返回当前 VMPC 压缩管线版本号（``"2.0"`` / ``"1.5"`` / ``"1.3"`` ...）。

        legacy 模式下，本字段决定 :func:`verse_torch.compress.compress_pipeline`
        走哪条压缩分支；VMPC 启用时强制为 ``"2.0"``（VSC + VN 联动）。
        """
        if self.is_vmpc_enabled():
            return "2.0"
        return str(self.vmpc_version)
